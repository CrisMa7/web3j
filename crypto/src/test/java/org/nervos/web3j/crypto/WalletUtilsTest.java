...
import static org.nervos.web3j.crypto.SampleKeys.CREDENTIALS;
import static org.nervos.web3j.crypto.SampleKeys.KEY_PAIR;
import static org.nervos.web3j.crypto.SampleKeys.PASSWORD;
import static org.nervos.web3j.crypto.WalletUtils.isValidAddress;
import static org.nervos.web3j.crypto.WalletUtils.isValidPrivateKey;
...
public class WalletUtilsTest {
    ...
    private void testGenerateNewWalletFile(String fileName) throws Exception {
        WalletUtils.loadCredentials(SampleKeys.PASSWORD, new File(tempDir, fileName));
    }
    ...
    private void testGenerateWalletFile(String fileName) throws Exception {
        Credentials credentials = WalletUtils.loadCredentials(
                SampleKeys.PASSWORD, new File(tempDir, fileName));

        assertThat(credentials, equalTo(SampleKeys.CREDENTIALS));
    }
    ...
    public void testIsValidPrivateKey() {
        assertTrue(WalletUtils.isValidPrivateKey(SampleKeys.PRIVATE_KEY_STRING));
        assertTrue(WalletUtils.isValidPrivateKey(Numeric.prependHexPrefix(SampleKeys.PRIVATE_KEY_STRING)));

        assertFalse(WalletUtils.isValidPrivateKey(""));
        assertFalse(WalletUtils.isValidPrivateKey(SampleKeys.PRIVATE_KEY_STRING + "a"));
        assertFalse(WalletUtils.isValidPrivateKey(SampleKeys.PRIVATE_KEY_STRING.substring(1)));
    }

    @Test
    public void testIsValidAddress() {
        assertTrue(WalletUtils.isValidAddress(SampleKeys.ADDRESS));
        assertTrue(WalletUtils.isValidAddress(SampleKeys.ADDRESS_NO_PREFIX));

        assertFalse(WalletUtils.isValidAddress(""));
        assertFalse(WalletUtils.isValidAddress(SampleKeys.ADDRESS + 'a'));
        assertFalse(WalletUtils.isValidAddress(SampleKeys.ADDRESS.substring(1)));
    }
}
